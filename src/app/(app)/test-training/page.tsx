    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 32 }}>
        {[
          ...
          { icon: <Trophy size={15} style={{ color: '#04FF88' }} />, bg: 'rgba(4,255,136,0.1)', label: 'Bester Part', value: bestPart ? (bestPart.section === 'LISTENING' ? `L · Part ${bestPart.part}` : bestPart.section === 'READING' ? `R · Part ${bestPart.part}` : bestPart.section === 'SPEAKING' ? `SP · ${bestPart.part}` : `WR · ${bestPart.part}`) : '—', sub: bestPart ? `${Math.round(bestPart.accuracy * 100)}% Genauigkeit` : 'Noch keine Daten' },
