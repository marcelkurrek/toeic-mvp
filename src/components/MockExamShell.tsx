...
export default function MockExamShell() {
  ...
  function handleSelect(key: string) { setSelected(key) }
  ...
  function handleNext() {
    ...
    if (nextPart !== q.part && parts.length > 1) {
      setPartBreak(nextPart)
    }
    setIndex(nextIndex)
  }
    ...
    return (
      <div style={{ maxWidth: 560, margin: '0 auto', paddingTop: 80 }}>
        <div className="card" style={{ padding: '48px 40px', textAlign: 'center' }}>
          ...
          <button onClick={() => router.push('/mock-exam')} className="btn-primary">
            Zurück zur Auswahl
          </button>
        </div>
      </div>
              ...
              <div key={p} className="card" style={{ padding: '16px 22px' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <span className="font-bold" style={{ color: pPct >= 80 ? 'var(--success)' : pPct >= 60 ? '#fbbf24' : 'var(--error)' }}>
                      {pPct}%
                    </span>
                  </div>
                </div>
                <div className="rounded-full overflow-hidden" style={{ height: 6, background: 'var(--card-border)' }}>
                  <div className="h-full rounded-full" style={{
                    width: `${pPct}%`,
                    background: pPct >= 80 ? 'var(--success)' : pPct >= 60 ? '#fbbf24' : 'var(--error)',
                  }} />
                </div>
              </div>
            )
              ...
              <div key={q.id} className="card" style={{ padding: '18px 22px' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {passage && (
                      <p className="text-xs" style={{ color: 'var(--muted)', marginBottom: 6, fontStyle: 'italic', lineHeight: 1.5 }}>
                    ...
                    {q.explanation && (
                      <p className="text-xs" style={{ marginTop: 8, color: 'var(--muted)', lineHeight: 1.5 }}>
                        <strong style={{ color: 'var(--accent)' }}>Erklärung:</strong> {q.explanation}
                      </p>
                    )}
                  </div>
                </div>
    ...
    return (
      <div style={{ maxWidth: 480, margin: '80px auto 0' }}>
        <div style={{
          ...
          background: timeLeft < 300 ? 'rgba(248,113,113,0.15)' : 'var(--card)',
          border: `1px solid ${timeLeft < 300 ? 'var(--error)' : 'var(--card-border)'}`,
          zIndex: 100,
        }}>
          <Clock size={14} style={{ color: timeLeft < 300 ? 'var(--error)' : 'var(--muted)' }} />
          <span className="font-mono font-bold" style={{ color: timeLeft < 300 ? 'var(--error)' : 'var(--foreground)' }}>
            {formatTime(timeLeft)}
          </span>
        </div>
        <div className="card" style={{ padding: '48px 40px', textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--muted)', lineHeight: 1.6, marginBottom: 28 }}>
            Du wechselst jetzt zum nächsten Abschnitt.
          </p>
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', paddingTop: 8 }}>
      ...
      <div style={{
        ...
        background: 'var(--background)', borderBottom: '1px solid var(--card-border)',
      }}>
        ...
        <div style={{
          display: 'flex', alignItems: 'center', gap: 6,
          padding: '6px 14px', borderRadius: 99,
          background: timeLeft < 300 ? 'rgba(248,113,113,0.15)' : 'var(--card-border)',
          border: `1px solid ${timeLeft < 300 ? 'var(--error)' : 'transparent'}`,
        }}>
          <Clock size={14} style={{ color: timeLeft < 300 ? 'var(--error)' : 'var(--muted)' }} />
          <span className="font-mono font-bold text-sm" style={{ color: timeLeft < 300 ? 'var(--error)' : 'var(--foreground)' }}>
            {formatTime(timeLeft)}
          </span>
        </div>
      </div>
        ...
        {passage && (
          <div style={{ borderLeft: '3px solid var(--accent)', paddingLeft: 16, marginBottom: 18 }}>
            ...
            <p className="text-sm" style={{ lineHeight: 1.7, whiteSpace: 'pre-wrap' }}>
              {passage.length > 500 ? passage.slice(0, 500) + '…' : passage}
            </p>
          </div>
        )}
        {questionText && (
          <p className="text-sm font-medium" style={{ lineHeight: 1.7, marginBottom: hasOptions ? 20 : 0 }}>
            {questionText}
          </p>
        )}
        {hasOptions && (
            ...
            {Object.entries(opts).map(([key, text]) => {
              ...
              return (
                <button key={key} onClick={() => handleSelect(key)} style={{
                  ...
                  border: `2px solid ${isChosen ? 'var(--accent)' : 'var(--card-border)'}`,
                  background: isChosen ? 'var(--accent-subtle)' : 'transparent',
